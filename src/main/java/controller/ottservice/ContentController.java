...
public class ContentController {
	...
	public void crawlAndSaveDramas() {
		try {
			driver.get("https://m.kinolights.com/discover/explore");
			...
			WebElement dramaTvButton = wait.until(ExpectedConditions.presenceOfElementLocated(
					By.xpath("//button[contains(@class, 'filter-category-btn') and contains(span/text(), '영화/TV')]")));
			((JavascriptExecutor) driver).executeScript("arguments[0].click();", dramaTvButton);

			WebElement dramaButton = wait.until(ExpectedConditions.elementToBeClickable(
					By.xpath("//button[contains(@class, 'filter-item-button') and span[text()=' TV']]")));
			dramaButton.click();
			
			WebElement dramaClickButton = wait.until(ExpectedConditions.elementToBeClickable(
					By.xpath("//button[contains(@class, 'filter-item-button') and span[text()=' 드라마']]")));
			dramaClickButton.click();
			...
			for (String url : detailLinks) {
				...
				try {
					...
					try {
						// "개봉일" 항목의 텍스트를 추출
						WebElement publishDateElement = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(
								"//li[@class='metadata__item']//span[@class='item__title' and contains(text(), '방영일')]/following-sibling::span")));
						String fullDateText = publishDateElement.getText().trim();
	...
	public void crawlAndSaveAnimations() {
		try {
			driver.get("https://m.kinolights.com/discover/explore");
			...
			WebElement dramaClickButton = wait.until(ExpectedConditions.elementToBeClickable(
					By.xpath("//button[contains(@class, 'filter-item-button') and span[text()=' 애니메이션']]")));
			dramaClickButton.click();
