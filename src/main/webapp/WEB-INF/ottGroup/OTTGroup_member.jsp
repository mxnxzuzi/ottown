...
<head>
    ...
    <link rel="stylesheet" href="<c:url value='/css/ottGroup_member.css' />" type="text/css">
</head>
<body>
    <!-- 헤더 포함 -->
    ...
    <main>
        ...
        <div class="room-top">
            ...
            <div class="room-top-box">
                <div class="room-top-name">
                    <p style="font-size: 20pt; color: #ED1C24; font-weight: bold;">${hostName}</p>
                    <p>님의 공동구매방</p>
                </div>
                <p style="margin: 0; font-size: 18pt;">${ottGroup.currentMembers} / 4</p>
            </div>
        </div>
        ...
        <div class="room-bottom">
            <form action="<c:url value='/ottGroup/member/isPaid' />" method="post">
                <input type="hidden" name="ottGroupMemberId" value="${ottGroupMemberId}" />
                <input type="hidden" name="ottGroupId" value="${ottGroup.id}" />
                <button type="submit" 
                        class="payment-button ${isPaid ? 'paid' : ''}" 
                        ${isPaid ? 'disabled' : ''}>
                    입금 완료
                </button>
            </form>
