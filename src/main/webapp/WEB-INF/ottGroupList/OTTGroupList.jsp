...
<body>
    ...
    <main>
        ...
        <div class="fixed-header">
            ...
            <a href="<c:url value='/ottGroup/createGroup?ottId=${ottService.id}' />" class="btn-room-create">방 개설하기</a>
        </div>

        <!-- 스크롤 가능한 공동구매방 리스트 -->
        <div class="group-room">
            <c:forEach var="room" items="${ottGroupList}">
                <div class="room" onclick="location.href='<c:url value='/ottGroup/OTTGroup_member?roomId=${room.id}' />'">
                    <div>
                        <!-- 호스트 이름을 hostNames 맵에서 가져오기 -->
                        <span class="name">${hostNames[room.id]}</span> 님의 공동구매방
                    </div>
